    ...
    <Box top={0} bottom={0} position={'sticky'} zIndex={99}>
      <Flex
        ...
        <Flex
          ...
          <IconButton onClick={onToggle} variant={'black'} aria-label={'Toggle Navigation'}
            icon={
        ...
        <Stack flex={{ base: 1, md: 0 }} justify={'flex-end'} direction={'row'} spacing={6}>
          ...
          {!isAuth?
          <Button as={'a'} fontSize={'xl'} fontWeight={400} color={'black'} variant={'link'}><Goto to='/login'><FiLogIn/></Goto>
          </Button>:<Text>{user.name}</Text>
          }
          {!isAuth &&
            <Button
              as={'a'}
