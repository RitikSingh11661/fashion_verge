...
export default function SingleProduct() {
  ...
  const cartData=JSON.parse(localStorage.getItem("cart"))||[]
   const handleCart=()=>{
    ...
    cartData.push(data)
    localStorage.setItem("cart",JSON.strngify(cartData))
    console.log(data)
   }
  return (
    <>
    ...
    <Box display={['block','block','flex','flex']} w={['100%','100%','80%',"80%"]} m='auto' gap='20px' mt='20px'>
     ...
     <Box textAlign={'left'} p='10px' >
        <Box display={'flex'} textAlign='left' justifyContent={'space-between'}>
         ...
         <Button display={['none','   ','block','block','block']} float={'right'} fontSize={'24px'}variant={'unstyled'} padding={'6px'} m='5px'
         ...
         <img style={{cursor:"pointer", margin:"10px 0px 20px 0px"}} src={data.size} alt="" />
         <Box>
