...
const SidebarWithHeader = ({ children }) => {
    ...
    const SidebarContent = ({ onClose, ...rest }) => {
        return (
            <Box transition="3s ease" bg={useColorModeValue('white', 'gray.900')} borderRight="1px"
                ...
                <Flex h="20" alignItems={"center"} justifyContent="center" mb='3vh'><Image width={'40'} src={logo} /></Flex>
                {LinkItems.map((link) => (<NavItem onClick={() => setComp(link.compName)} key={link.name} icon={link.icon}>{link.name}</NavItem>))}
            </Box>
        );
    };
    ...
    // console.log('Admin Sidebar is rendering');
    ...
    return (
        <Box>
            <Box minH="100vh" bg={useColorModeValue('gray.100', 'gray.900')}>
                ...
                <Flex className='main-content' justifyContent={'center'} ml='100'>{componentChange()}</Flex>
